...
At this time I am developing QLogo on Linux (Fedora 43) using Qt 6.10 and LLVM 21.1.

I just now updated my LLVM version from 19 to 21 on Linux, so this *PARTICULAR* version does not build for me on macOS (Tahoe on Silicon, using LLVM 19 and Qt 6.9 provided by MacPorts).
