//===-- qlogo/logo_main.cpp - main function for QLogo -------*- C++ -*-===//
//
// Copyright 2017-2024 Jason Sikes
//
// Redistribution and use in source and binary forms, with or without
// modification, are permitted under the conditions specified in the
// license found in the LICENSE file in the project root.
//
//===----------------------------------------------------------------------===//
///
/// \file
/// This file contains the main function for the QLogo language interpreter. Upon
/// initialization, the main function will process command line arguments and create
/// a LogoInterface (or LogoInterface subclass) object. The main function will then
/// call the run() method of the Kernel object, which will start the main
/// loop of the QLogo language interpreter.
///
//===----------------------------------------------------------------------===//

#include "interface/logointerfacegui.h"
#include "sharedconstants.h"
#include "workspace/kernel.h"
#include <QCommandLineParser>
#include <QCoreApplication>
#include <memory>

/// Process command line options and set the configuration accordingly.
///
/// @param a The QCoreApplication object.
void processOptions(QCoreApplication *a)
{
    QString optPsychi = "Psychi";
    QString optsetlibloc = "setlibloc";
    QString optsethelploc = "sethelploc";
    QString optshowIR = "showIR";
    QString optshowCFG = "showCFG";
    QString optshowModuleIR = "showModuleIR";
    QString optverifyIR = "verifyIR";
    QString optshowCON = "showCON";
    QString optTraceEvaluator = "traceEvaluator";
    const char *mainString = "main";

    QCommandLineParser commandlineParser;

    QCoreApplication::setApplicationName("qlogo");
    QCoreApplication::setApplicationVersion(LOGOVERSION);

    commandlineParser.setApplicationDescription("UCBLOGO-compatible Logo language Interpreter.");
    commandlineParser.addHelpOption();
    commandlineParser.addVersionOption();

    commandlineParser.addOptions({
        {optPsychi,
         QCoreApplication::translate(mainString,
                                     "DO NOT USE! Sets the input and output to the format used by "
                                     "the Psychi Application. Will result in garbage being printed "
                                     "out to your terminal.")},

        {optsetlibloc,
         QCoreApplication::translate(mainString, "Specify the location of the standard library database."),
         QCoreApplication::translate("main", "library_database")},
        {optsethelploc,
         QCoreApplication::translate(mainString, "Specify the location of the help database."),
         QCoreApplication::translate("main", "help_database")},
        {optshowIR,
         QCoreApplication::translate(mainString,
                                     "Show the IR code generated by llvm before coroutine lowering. "
                                     "(for debugging).")},
        {optverifyIR,
         QCoreApplication::translate(mainString,
                                     "Verify the IR code generated by llvm. "
                                     "(for debugging).")},
        {optshowCFG,
         QCoreApplication::translate(mainString,
                                     "Show the CFG graph generated by llvm. "
                                     "(for debugging).")},
        {optshowModuleIR,
         QCoreApplication::translate(mainString,
                                     "Show the full module IR after coroutine lowering (each function separately). "
                                     "(for debugging).")},
        {optshowCON,
         QCoreApplication::translate(mainString,
                                     "Show every change in the Count Of Nodes. "
                                     "(for debugging).")},
        {optTraceEvaluator,
         QCoreApplication::translate(mainString,
                                     "Trace evaluator execution (for debugging).")},
    });

    commandlineParser.process(*a);

    if (commandlineParser.isSet(optPsychi))
    {
        Config::get().hasGUI_ = true;
    }

    if (commandlineParser.isSet(optsetlibloc))
    {
        Config::get().paramLibraryDatabaseFilepath_ = commandlineParser.value(optsetlibloc);
    }

    if (commandlineParser.isSet(optsethelploc))
    {
        Config::get().paramHelpDatabaseFilepath_ = commandlineParser.value(optsethelploc);
    }

    if (commandlineParser.isSet(optshowIR))
    {
        Config::get().showIR_ = true;
    }

    if (commandlineParser.isSet(optverifyIR))
    {
        Config::get().verifyIR_ = true;
    }

    if (commandlineParser.isSet(optshowCFG))
    {
        Config::get().showCFG_ = true;
    }

    if (commandlineParser.isSet(optshowModuleIR))
    {
        Config::get().showModuleIR_ = true;
    }

    if (commandlineParser.isSet(optshowCON))
    {
        Config::get().showCON_ = true;
    }

    if (commandlineParser.isSet(optTraceEvaluator))
    {
        Config::get().traceEvaluator_ = true;
    }
}

int main(int argc, char **argv)
{
    // Pass all the command line arguments to Config in case they are queried later.
    for (int i = 0; i < argc; ++i)
    {
        Config::get().ARGV_.push_back(QString(argv[i]));
    }

    QCoreApplication application(argc, argv);

    processOptions(&application);

    std::unique_ptr<LogoInterface> mainInterface;
    if (Config::get().hasGUI_)
    {
        mainInterface = std::make_unique<LogoInterfaceGUI>();
    }
    else
    {
        mainInterface = std::make_unique<LogoInterface>();
    }
    int retval = Kernel::get().run();
    return retval;
}
